...
def test_init_creates_output_directory_and_writer(temp_video_path):
    ...
    THEN the directory should be created and the
    video writer opened successfully.
    """
    processor = ArchiveProcessor(output_path=temp_video_path,
                                 fps=1,
                                 frame_size=(2, 2)
                                 )
...
def test_init_raises_io_error_when_video_writer_fails(
        monkeypatch,
        temp_video_path
        ):
    """
